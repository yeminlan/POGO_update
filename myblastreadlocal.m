function data = myblastreadlocal(blastreport, mopt)
...
for i = 1:numQueries
    ...
    if isempty(strfind(queryReport,'No hits found *****'))
...
use_positives = false;
use_strand = false;
if strfind(blasttext,'Frame =') % Frame is included in translated searches
    use_frame = true;
end
if strfind(blasttext,'Positives =') % Positives are included with peptide sequences
    use_positives = true;
end
...
for i = 1:numQueries
    ...
    if isempty(strfind(queryReport,'No hits found *****'))
        ...
        for j = 1: numSubjects
            ...
            hspsExpect = regexpi(subjectReport, 'Expect\(*\d*\)*\s+=\s{1,2}([e-\d\.]*)','tokens');
            ...
            [hspsScores, hspsScoreStarts] = regexpi(subjectReport, ...
                'Score =\s{1,2}([e+\d\.]*)','tokens', 'start');
